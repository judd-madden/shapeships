...
- Stable Mission Finding IDs linking authored Missions to Lore > Mission Findings.
- Session-scoped tracking of Mission Findings actually seen by the player.
- Default blurred Mission Findings in Lore, revealed when an associated Mission is visibly presented.
...
- Rewards, XP, gameplay unlocks, currency, badges, leaderboards, or account-backed progression.
...
- Permanent/account-backed Mission Finding persistence or cross-device discovery.
...
## 5.9 Lore Mission Findings are static client-owned rows

The current Lore implementation owns Mission Findings as static repository content on the client.

Phase 15 should preserve that posture. Mission Finding rows may receive stable IDs and read local discovery state, but Lore prose does not need to move to canonical server state.
...
## 16.4 Mission Finding discovery helper

The client also owns a small session-scoped set of seen Mission Finding IDs.
...
Seen Mission Findings are local presentation/discovery state only. They are not canonical game state, achievements, rewards, or authoritative progression.

## 16.5 Desktop/mobile share truth
...
- the same visible/open Mission Finding discovery behavior using the shared session helper.
...
Connect the existing Lore Mission Findings rows to the session discovery state.

### Includes

- stable IDs on existing Mission Finding rows;
- default approximately 20px blur for unseen rows;
- clear rendering for seen rows;
- unchanged row dimensions/layout when revealing;
- read discovery state through one focused client helper/seam;
- preserve existing Lore content, navigation, and responsive structure.

### Out of scope

- account persistence;
- server progression;
- secure server-side content withholding;
- gameplay rewards;
...
- unseen Lore rows render blurred at approximately 20px;
- seen Lore rows render clearly without material layout shift.
